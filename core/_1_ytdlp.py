...
def find_video_files(save_path='output'):
    ...
    search_base = Path(save_path)
    if search_base.is_absolute():
        search_dirs = [search_base]
    else:
        search_dirs = [(Path.cwd() / search_base), (project_root / search_base)]

    # Keep order and deduplicate by resolved path.
    seen = set()
    dedup_dirs = []
    ...
    allowed_exts = set(load_key("allowed_video_formats"))
    video_files = []
    for base_dir in dedup_dirs:
        if not base_dir.exists():
            continue
        for file in glob.glob(str(base_dir / "*")):
            ext = os.path.splitext(file)[1][1:].lower()
            if ext in allowed_exts:
                video_files.append(file)

    # Exclude generated output videos from input candidates.
    video_files = [
        file for file in video_files
        if not os.path.basename(file).startswith("output_")
    ]

    # Normalize slashes on Windows for downstream tools.
    if sys.platform.startswith('win'):
        video_files = [file.replace("\\", "/") for file in video_files]

    if len(video_files) == 0:
        searched = ", ".join(str(d) for d in dedup_dirs)
        raise ValueError(
            f"No input video found in [{searched}]. "
            "Please upload/download a video in step a."
        )

    if len(video_files) > 1:
        # Prefer the most recently modified file to avoid blocking the pipeline.
        selected = max(video_files, key=lambda f: os.path.getmtime(f))
        rprint(
            f"[yellow]⚠️ Multiple input videos found ({len(video_files)}). "
            f"Using the newest: {selected}[/yellow]"
        )
        return selected
